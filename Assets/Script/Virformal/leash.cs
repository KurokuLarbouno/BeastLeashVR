...
public class leash : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (LeashVec.magnitude > leashLimit) //繃直
        {
            ...
            Dog.transform.position += new Vector3(tmpVec.x, 0, tmpVec.z);//狗退，到下次到位置再走
            //Dog.GetComponent<FormalDog>().isFallback = true;
            //震動(0, a*幅度. a*幅度. 0.1sec)
            //Debug.Log(acc);
            ...
            if (VirtualDog.GetComponent<FormalDog>().dogState == 1) VibrateManerger.GetComponent<VibrationManager>().Pulse(0.1f, 250 * flexpart, 100 * flexpart, SteamVR_Input_Sources.RightHand);//Pulse(float duration, float frequency, float amplitude, SteamVR_Input_Sources sources)
        }
        else
        {
            VirtualDog.GetComponent<FormalDog>().isFallback = false;
